...
echo -e "$white\033[0;34m└─────────────────────────────────────────┘${NC}"
sleep 1
wget -q https://raw.githubusercontent.com/KedaiVPN/dynamic/main/ssh-vpn2.sh && chmod +x ssh-vpn2.sh && ./ssh-vpn2.sh
#install ins-xray
echo -e "$white\033[0;34m┌─────────────────────────────────────────┐${NC}"
...
cd /root/limit
    git config --global user.email "example@gmail.com" &> /dev/null
    git config --global user.name "username_github" &> /dev/null
    rm -fr .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    ...
    git push -f https://${GITHUB_TOKEN}@github.com/kenDevXD/limit.git &> /dev/null
cd
echo "1.1" >> /home/.ver
rm -fr /root/limit
